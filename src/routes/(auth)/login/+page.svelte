  ...
  <div class="w-full max-w-4xl">
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
      <div class="grid grid-cols-1 md:grid-cols-2">
        <div class="hidden md:flex items-center justify-center bg-gray-50 p-6">
          <img src="/logo/logo_bnpt.png" alt="BNPT" class="w-3/4 h-auto" />
        </div>
        <div class="p-8">
          <div class="text-center">
            <img src="/logo/logo-pasti-portal.png" alt="PASTI" class="w-full h-auto object-contain" />
            <hr class="my-4" />
          </div>
          <form method="POST" use:enhance class="space-y-4">
            ...
            <div>
              <div class="relative">
                ...
                />
                <button type="button" class="absolute inset-y-0 right-0 px-3 text-gray-600" on:click={togglePasswordVisibility} aria-label="Toggle password visibility">
                  {#if showPassword}
            ...
            <button type="submit" class="w-full bg-gray-900 text-white py-2 rounded-md hover:bg-black transition-colors">Login</button>
          </form>

          <hr class="my-6" />
          <div class="text-center text-gray-600 text-sm">&copy; BNPT Subdit Teknologi Intelijen {new Date().getFullYear()}</div>
        </div>
      </div>
    </div>
  </div>
