...
<div class="space-y-6">
    ...
    <div class="mb-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
      <input
        ...
        class="w-full px-3 py-2 sm:w-60 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none focus:ring-2"
        bind:value={searchInput}
      />

      <select
        class="w-full px-3 py-3 sm:w-52 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none focus:ring-2"
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
          {#each paginatedUsers as u}
            <tr>
              ...
              <!-- <td class="px-4 py-2 text-sm text-gray-600">{(u.permissions || []).slice(0,3).join(', ')}{#if (u.permissions || []).length > 3}…{/if}</td> -->
               <td class="px-4 py-2 text-sm text-gray-600">
                <div class="flex flex-wrap gap-1">
                  {#each (u.permissions || []).slice(0, 5) as perm}
                    <span class="inline-flex items-center rounded bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-800 ring-1 ring-inset ring-gray-300">
                      {perm}
                    </span>
                  {/each}
                  {#if (u.permissions || []).length > 5}
                    <span class="inline-flex items-center rounded bg-gray-200 px-2 py-0.5 text-xs font-medium text-gray-600">
              ...
              <td class="px-4 py-2 text-right">
                <a class="text-indigo-600 hover:underline text-sm" href="/user-management/user/{u.username}">Detail</a>
              </td>
            </tr>
          {/each}
        </tbody>
          ...
          {#each Array(totalPages).fill(0).map((_, i) => i + 1) as page}
            <button
              ...
              class="px-3 py-1 text-sm rounded-md border border-gray-300 hover:bg-gray-100
                {page === currentPage ? 'bg-indigo-600 text-white border-indigo-600' : 'text-gray-700'}"
